...
function single_drama($content){
    ...
    if ($drama['star_cast']) {
        ...
        if ($tmp_starcast) {
            ...
            $data .= '<h2 class="images-title weight-700 font-size-36">'.$title.' Star Cast</h2>';
            $data .= '</div>';
    ...
    if($content):
        ...
        $data .= '<div class="short-heading font-size-36 weight-700">';
        $data .= '<h2 class="images-title weight-700 font-size-36">'.$title.' Drama Story</h2>';
        $data .= '</div>';
    ...
    $data .= '<div class="short-heading font-size-36 weight-700">';
    $data .= '<h2 class="images-title weight-700 font-size-36">'.$title.' Cast & Crew</h2>';
    $data .= '</div>';
...
function drama_details_table($drama, $director, $director_job, $producer, $producer_job, $writer, $musician, $director_name, $producer_name,$writer_name,$musician_name){
    ...
    $data .= '<tr class="detail-item"><th class="item-title th">Status</th><td class="item-title">'.($drama['release_timestamp'] > time() ? 'Upcoming' : ($drama['finished'] ? 'Ended' : 'Ongoing')).'</td></tr>';
    if($drama[ 'release_date' ]) $data .= '<tr class="detail-item"><th class="item-title th">Release Date</th><td class="item-title">'.date( 'd F Y', $drama['release_timestamp'] ).'</td></tr>';
    if($drama[ 'original_title' ]) $data .= '<tr class="detail-item"><th class="item-title th">Original Title</th><td class="item-title">'.$drama[ 'original_title' ].'</td></tr>';
    if($drama[ 'tagline' ]) $data .= '<tr class="detail-item"><th class="item-title th">Tagline</th><td>'.$drama[ 'tagline' ].'</div></div>';
    if($linked_langs = get_linked_terms('language')) $data .= '<tr class="detail-item"><th class="item-title th">Language</th><td class="item-title" title="'.get_the_terms(get_the_ID(), 'language')[0]->name.' Dramas">'.$linked_langs.'</td></tr>';
    if($country = get_linked_terms('country')) '<tr class="detail-item"><th class="item-title th">Country</th><td title="List of All '.get_the_terms(get_the_ID(), 'country')[0]->name.' Dramas">'.$country.'</td></tr>';
    if($linked_genres = get_linked_terms('genre')) $data .= '<tr class="detail-item"><th class="item-title th">Genre</th><td class="item-title">'.$all_generes.'</td></tr>';
    if($channel) $data .= '<tr class="detail-item"><th class="item-title th">Channel</th><td title="'.$channel_name.' Dramas">'.$channel.'</td></tr>';
    if($director) $data .= '<tr class="detail-item"><th class="item-title th">'.$director_job.'</th><td title="'.$director_name.' Dramas">'.$director.'</td></tr>';
    if($writer) $data .= '<tr class="detail-item"><th class="item-title th">Writer</th><td title="'.$writer_name.' Dramas">'.$writer.'</td></tr>';
    if($producer) $data .= '<tr class="detail-item"><th class="item-title th">'.$producer_job.'</th><td title="'.$producer_name.' Dramas">'.$producer.'</td></tr>';
    if($drama[ 'runtime' ]) $data .= '<tr class="detail-item"><th class="item-title th">Duration</th><td>'.$drama[ 'runtime' ].'</td></tr>';
    if($drama[ 'schedule_timestamp' ] && $drama['schedule_day'] && !$drama['finished']) $data .= '<tr class="detail-item"><th class="item-title th">Schedule</th><td>'.(($total_sd = count($drama['schedule_day'])) === 7 ? 'Daily' : 'Every ').($total_sd === 1 ? $drama['schedule_day'][0] : ($total_sd === 2 ? implode(' and ', $drama['schedule_day']) : ($total_sd !== 7 ?findMissingDaysAndConsecutiveRange($drama['schedule_day']) : ''))).' at '.date("h:i A", $drama['schedule_timestamp']).' PST</div></div>';
    if($drama['total_episodes']) $data .= '<tr class="detail-item"><th class="item-title th">No. of Episodes</th><td>'.$drama['total_episodes'].'</td></tr>';
    if($drama[ 'production_house' ]) $data .= '<tr class="detail-item"><th class="item-title th">Production House</th><td title="'.$drama[ 'production_house' ].' Dramas">'.$drama[ 'production_house' ].'</td></tr>';
    if($musician) $data .= '<tr class="detail-item"><th class="item-title th">Music</th><td title="'.$musician_name.' Dramas">'.$musician.'</td></tr>';
    if($drama[ 'certification' ]) $data .= '<tr class="detail-item"><th class="item-title th">Certificate</th><td>'.$drama[ 'certification' ].'</td></tr>';
    $data .= '</table>';
    $data .= '</div>';
    return $data;
}
